...
def nuscenes_data_prep(root_path,
    ...
    if version == 'v1.0-test':
        ...
        info_test_path = osp.join(root_path, f'{info_prefix}_infos_raw.pkl')
...
def mv_waymo_data_prep(root_path,
    ...
    Args:
        ...
        max_sweeps (int): Number of input consecutive frames. Default: 5 \
            Here we store pose information of these frames for later use.
    """
    from tools.data_converter import mv_waymo_converter as waymo
    ...
    for i, split in enumerate(splits):
        load_dir = osp.join(root_path, 'waymo_format', split)
        if split == 'validation':
            save_dir = osp.join(out_dir, 'mv_waymo_format', 'training')
        else:
            save_dir = osp.join(out_dir, 'mv_waymo_format', split)
        converter = waymo.Waymo2Nucenes(
            load_dir,
            save_dir,
            prefix=str(i),
            workers=workers,
    ...
    # Generate waymo infos
    # out_dir = osp.join(out_dir, 'mv_waymo_format')
    # kitti.create_waymo_info_file(out_dir, info_prefix, max_sweeps=max_sweeps)
    #create_groundtruth_database(
    #    'WaymoDataset',
    #    out_dir,
...
if __name__ == '__main__':
    ...
    elif args.dataset == 'nuscenes' and args.version == 'v1.0-raw':
