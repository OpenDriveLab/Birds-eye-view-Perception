...
class BEVTransformerOnlyEncoder(BaseModule):
    ...
    def __init__(
            ...
            **kwargs):
        super(BEVTransformerOnlyEncoder, self).__init__(**kwargs)
    ...
    def forward(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if self.use_cams_embeds:
                if kwargs['img_metas'][0].get('flip', False):
                    tmp_cams_embeds = self.cams_embeds[[0, 2, 1, 4, 3]]
                else:
                    tmp_cams_embeds = self.cams_embeds
                feat = feat + tmp_cams_embeds[:, None, None, :].to(feat.dtype)
            else:
