...
def get_iou(gt_box, predicted_box):
    c1 = np.concatenate(
        [gt_box.translation, np.array(gt_box.size)[[0, 2, 1]], [Quaternion(gt_box.rotation).yaw_pitch_roll[0]]])
    c2 = np.concatenate([list(predicted_box.translation), np.array(list(predicted_box.size))[[0, 2, 1]],
                         [Quaternion(list(predicted_box.rotation)).yaw_pitch_roll[0]]])
    iou = c_iou(torch.tensor(c1)[None, ...], torch.tensor(c2)[None, ...])
    return 1 - iou
...
def filter_by_sample_token(ori_eval_boxes, valid_sample_tokens=[],  verbose=False):
    ...
    # eval_boxes.sample_tokens = list(eval_boxes.boxes.keys())
    return eval_boxes
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def bottom_corners(self, corners) -> np.ndarray:
        """
        Returns the four bottom corners.
        """
        return corners[:, [0, 3, 7, 4]]

    def prepare_bbox_mask_gt(self, tokens):

        bbox_mask_list = []
        for sample_token in tokens:
            sample = self.nusc.get('sample', sample_token)
            map_mask = self.Bbox_mask.get_binimg(sample)
            bbox_mask_list.append(map_mask)
        map_masks = np.stack(bbox_mask_list)
        return map_masks

    def prepare_map_mask_gt(self, tokens):
        map_mask_list = []
        for sample_token in tokens:
            sample = self.nusc.get('sample', sample_token)
            map_mask = self.HDMap.get(sample)
            map_mask_list.append(map_mask)
        map_masks = np.stack(map_mask_list)
        return map_masks
    ...
    def get_batch_iou(self, pred, gt):
        ...
        with torch.no_grad():
            ...
            # iou_list = []
            # for i, j in zip(intersect, union):
            #    iou = i/j if (j>0) else torch.tensor([1.])
            #    iou_list.append(iou.cpu().item())
        return (intersect / union).cpu().item()
