...
class LidarWaymoDataset(KittiDataset):
    """Waymo Dataset.
    ...
    ###
    def _convert_valid_bboxes(self, box_dict, info):
        ...
        img_filename_0 = os.path.join(self.data_root,
                                      info['image']['image_path'])
        image_filenames = []
        for i in range(5):
            image_filenames.append(img_filename_0.replace('image_0', f'image_{i}'))
        calib_file = img_filename_0.replace('image_0', 'calib').replace('png', 'txt')
        ...
        for i in range(5):
            ...
            box_2d_preds = torch.cat([minxy, maxxy], dim=1)
            # Post-processing
            a = ((box_corners_in_image[..., 0] < 0) | (box_corners_in_image[..., 0] > img_shape[1]))
            b = ((box_corners_in_image[..., 1] < 0) | (box_corners_in_image[..., 1] > img_shape[0]))
            a = ~a
            b = ~b
            c = a & b
            c = c.sum(-1) > 4
        ...
        if len(valid_inds) != valid_inds.sum():
            print(len(valid_inds), valid_inds.sum())
