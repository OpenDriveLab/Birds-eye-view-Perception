...
class CustomCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'can_bus', 'lidar2ego_translation', 'lidar2ego_rotation', # add for projection
                            )):
        self.keys = keys
...
@PIPELINES.register_module()
class CustomMaskMultiViewImage(object):
    def __init__(self, mask_ratio):
        self.mask_ratio = mask_ratio
    
    def _mask_img(self, results):
        masked_imgs = []
        for idx, img in enumerate(results['img']):
            mask_pixels = int(img.shape[0] * self.mask_ratio[idx])
            _img = img.copy()
            _img[:mask_pixels, :, :] = 0
            masked_imgs.append(_img)
        results['img'] = masked_imgs

    def __call__(self, results):
        self._mask_img(results)
        return results