...
def obtain_bboxes_on_lidar(nusc: NuScenes,
                     sample_token: str,
                     gt_boxes: EvalBoxes,
    ...
    return boxes_est, boxes_gt

def bboxes_lidar_view(sample_token, data):
    bbox_gt_list = []
    bbox_pred_list = []
    ...
    return obtain_bboxes_on_lidar(nusc, sample_token, gt_annotations, pred_annotations)

def visualize_sample_on_lidar(nusc: NuScenes,
                     sample_token: str,
                     gt_boxes: EvalBoxes,
                     pred_boxes: EvalBoxes,
                     ax: Axes,
...
def lidiar_render(sample_token, data, ax_lidar, out_path=None):
    bbox_gt_list = []
    bbox_pred_list = []
    anns = nusc.get('sample', sample_token)['anns']
    for ann in anns:
    ...
    visualize_sample_on_lidar(nusc, sample_token, gt_annotations, pred_annotations, ax_lidar, savepath=out_path+'_bev')
...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load('test/bevformer_base/Mon_Sep_19_15_49_55_2022/pts_bbox/results_nusc.json')
    sample_token_list = list(bevformer_results['results'].keys())
    for id in range(0, len(bevformer_results['results'].keys())):
        if sample_token_list[id] == '747aa46b9a4641fe90db05d97db2acea':
            render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=sample_token_list[id])
            
    '''
    bevformer_results['results']['747aa46b9a4641fe90db05d97db2acea'][0]:
    {
        'sample_token': '747aa46b9a4641fe90db...d97db2acea', 
        'translation': [600.8443175919239, 1639.9504595964902, 1.2953847166146197], 
        'size': [0.6200377941131592, 0.7212116122245789, 1.755219578742981], 
        'rotation': [0.9570364818338065, 0.01887160458885498, 0.04238645417906104, -0.28623141595837837], 
        'velocity': [1.4761182274225029, -0.9576058263658647], 
        'detection_name': 'pedestrian', 
        'detection_score': 0.9589961767196655, 
