...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                # FIXME : online training use `bevdet` rather than `bevdet4d`, but adjancet is necessary in some cases.
                input_dict.update(dict(adjacent=[])) 
                input_dict.update(dict(adjacent_ann=[]))
                info_adj_list, ann_adj_list = self.get_adj_info(info, index)
                input_dict.update(dict(motion=info_adj_list))
                input_dict.update(dict(motion_ann=ann_adj_list))    
                
                if '4d' in self.img_info_prototype:
                    info_adj_list, ann_adj_list = self.get_adj_info(info, index)
                    input_dict.update(dict(adjacent=info_adj_list))
                    input_dict.update(dict(adjacent_ann=ann_adj_list))
            if self.use_sequence_group_flag:
    ...
    def get_adj_info(self, info, index):
        info_adj_list = []
        ann_adj_list = []
        for idx, select_id in enumerate(range(*self.multi_adj_frame_id_cfg)):
            if self.key_frames !=0 and idx == self.key_frames:
                continue
            select_id = max(index - select_id, 0)
            select_id = min(select_id, len(self.data_infos) - 1)
            if not self.data_infos[select_id]['scene_token'] == info[
                    'scene_token']:
                info_adj_list.append(info_adj_list[-1] if len(info_adj_list)>0 else info)
                ann_adj_list.append(ann_adj_list[-1] if len(ann_adj_list)>0 else info['ann_infos'])
            else:
                info_adj_list.append(self.data_infos[select_id])
                ann_adj_list.append(self.data_infos[select_id]['ann_infos'])
        return info_adj_list, ann_adj_list
