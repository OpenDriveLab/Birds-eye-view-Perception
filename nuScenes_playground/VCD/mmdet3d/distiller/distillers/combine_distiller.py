...
class CombineDistiller(BaseDetector):
    ...
    def __init__(self,
                 ...
                 distill_on_occupancy = False,
                 distill_PV = None,
                 distill_BEV = None,
                 distill_depth = None,
                 distill_occupancy = None,
                 channel_PV = 64,
                 channel_BEV = 256,
                 channel_depth = 64,
        ...
        super(CombineDistiller, self).__init__()
        ...
        self.distill_on_occupancy = distill_on_occupancy
        if self.distill_on_occupancy:
            self.distill_losses_occupancy = nn.ModuleDict()
            self.distill_occupancy = distill_occupancy
            if distill_occupancy is not None:
                for item_loc in distill_occupancy:
                    for item_loss in item_loc.methods:
                        loss_name = item_loss.name
                        self.distill_losses_occupancy[loss_name] = build_distill_loss(item_loss)
        ...
        if 'occ_resolution' in kwargs.keys():
            self.occ_resolution = kwargs['occ_resolution']
        else:
            self.occ_resolution = {'x': [-51.2, 51.2, 0.4], 'y': [-51.2, 51.2, 0.4], 'z': [-5, 3, 1]}
    ...
    def forward_train(self, **kwargs):
        ...
        if self.distill_on_occupancy:
            if self.distill_occupancy is not None:
                with torch.no_grad():
                    occupancy_tch = self.teacher.img_view_transformer.obtain_occupancy(depth_fusion_tch, kwargs['img_inputs'], self.occ_resolution)
                    occupancy_tch = torch.sigmoid(occupancy_tch) - 0.5
                occupancy_stu = self.student.img_view_transformer.obtain_occupancy(pred_depth_stu, kwargs['img_inputs'], self.occ_resolution)
                occupancy_stu = torch.sigmoid(occupancy_stu) - 0.5
                occupancy_heatmap = self.teacher.pts_bbox_head.get_3DHeatmaps(kwargs['gt_bboxes_3d'], kwargs['gt_labels_3d']) + 1e-7
                occupancy_stu = occupancy_stu * occupancy_heatmap
                occupancy_tch = occupancy_tch * occupancy_heatmap
                for item_loc in self.distill_occupancy:
                    for item_loss in item_loc.methods:
                        loss_name = item_loss.name
                        student_loss[loss_name] = (1/occupancy_heatmap.sum()) * self.distill_losses_occupancy[loss_name](occupancy_stu, occupancy_tch)
