...
def gaussian_3d(shape, sigmas):
    ...
    x, y, z = np.ogrid[-m:m + 1, -n:n + 1, -k:k+1]

    h = np.exp(-(x * x + y * y + z * z) / (sigma_x**2 + sigma_y**2 + sigma_z**2))
    h[h < np.finfo(h.dtype).eps * h.max()] = 0
    return h
...
def draw_heatmap_gaussian_3D(heatmap, center, radius, k=1):
    """Get gaussian masked heatmap.
    ...
    Returns:
        torch.Tensor: Masked heatmap.
    """
    diameter_x = 2 * radius[0] + 1
    diameter_y = 2 * radius[1] + 1
    diameter_z = 2 * radius[2] + 1
    gaussian = gaussian_3d((diameter_x, diameter_y, diameter_z), sigmas=(diameter_x/6, diameter_y/6, diameter_z/6))

    x, y, z = int(center[0]), int(center[1]), int(center[2])

    length, width, height  = heatmap.shape[0:3]

    left, right = min(x, radius[0]), min(width - x, radius[0] + 1)
    far, near = min(y, radius[1]), min(length - y, radius[1] + 1)
    top, bottom = min(z, radius[2]), min(height - z, radius[2] + 1)

    masked_heatmap = heatmap[y - far:y + near, x - left:x + right, z-top:z+bottom]
    masked_gaussian = torch.from_numpy(
        gaussian[radius[0] - far:radius[0] + near,
                 radius[1] - left:radius[1] + right,
                 radius[2] - top:radius[2] + bottom,]).to(heatmap.device, torch.float32)
    if min(masked_gaussian.shape) > 0 and min(masked_heatmap.shape) > 0:
        torch.max(masked_heatmap, masked_gaussian * k, out=masked_heatmap)
    return heatmap
