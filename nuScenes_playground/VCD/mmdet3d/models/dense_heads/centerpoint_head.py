...
from mmdet3d.core import (circle_nms, draw_heatmap_gaussian, draw_heatmap_gaussian_3D, gaussian_radius,
                          xywhr2xyxyr)
...
class CenterHead(BaseModule):
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d):
        ...
        heatmaps, anno_boxes, inds, masks = multi_apply(
            self.get_targets_single, gt_bboxes_3d, gt_labels_3d) # [LiDARInstance3DBoxes...32e-02]]))], [tensor([8, 8, 0, 8, ...='cuda:0')]
        # Transpose heatmaps
        heatmaps = list(map(list, zip(*heatmaps))) # [[torch.Size([1, 128, 128])] * bs] * n_tasks
        heatmaps = [torch.stack(hms_) for hms_ in heatmaps] # [torch.Size([bs, 1, 128, 128])] * n_tasks
    ...
    def get_heatmaps(self, gt_bboxes_3d, gt_labels_3d):
        """Generate targets.
    ...
    def get_heatmaps_single(self, gt_bboxes_3d, gt_labels_3d):
        """Generate training targets for a single sample.
    ...
    def get_3DHeatmaps(self, gt_bboxes_3d, gt_labels_3d):
        """
        Args:
            gt_bboxes_3d (list[:obj:`LiDARInstance3DBoxes`]): Ground
                truth gt boxes.
        ...
        heatmaps3D = multi_apply(
            self.get_3DHeatmaps_single, gt_bboxes_3d, gt_labels_3d) # [LiDARInstance3DBoxes...32e-02]]))], [tensor([8, 8, 0, 8, ...='cuda:0')]
        # Transpose heatmaps
        heatmaps3D = list(map(list, zip(*heatmaps3D)))
        heatmaps3D = [torch.stack(hms_) for hms_ in heatmaps3D] # [torch.Size([bs, 256, 256, 8])]
        return heatmaps3D[0]

    def get_3DHeatmaps_single(self, gt_bboxes_3d, gt_labels_3d):
        """Generate training targets for a single sample.
        ...
        max_objs = self.train_cfg['max_objs'] * self.train_cfg['dense_reg']
        grid_size = torch.tensor(self.train_cfg['grid_size'])
        pc_range = torch.tensor(self.train_cfg['point_cloud_range']) # tensor([-51.2, -51.2,  -5.0,  51.2,  51.2,   3.0])
        voxel_size = torch.tensor(self.train_cfg['voxel_size']) # tensor([0.1, 0.1, 0.2])
        ...
        heatmap_occupancy = torch.zeros((range_x[1]+1, range_y[1]+1, range_z[1]+1)).to(device)
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    radius_x = radius
                    radius_y = radius
                    radius_z1 = gaussian_radius((length, height),
                            min_overlap=self.train_cfg['gaussian_overlap'])
                    radius_z2 = gaussian_radius((width, height),
                            min_overlap=self.train_cfg['gaussian_overlap'])
                    radius_z = max(radius_z1, radius_z2)
                    radius_x = int(radius_x/occ_resolution['x'][2])
                    radius_y = int(radius_y/occ_resolution['y'][2])
                    radius_z = int(radius_z/occ_resolution['z'][2])
                    ...
                    coor_x = (
                        x - occ_resolution['x'][0]
                    ) / occ_resolution['x'][2]
                    coor_y = (
                        y - occ_resolution['y'][0]
                    ) / occ_resolution['y'][2]
                    coor_z = (
                        z - occ_resolution['z'][0]
                    ) / occ_resolution['z'][2]

                    center = torch.tensor([coor_x, coor_y, coor_z],
                                          dtype=torch.float32,
                                          device=device)
                    center_int = center.to(torch.int32)
