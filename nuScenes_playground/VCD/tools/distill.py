...
from mmdet3d.apis.train import custom_distill_model
from mmdet3d.distiller import build_distiller
...
def main():
    ...
    for each in cfg.log_config['hooks']:
        if each['type'] == 'WandbLoggerHook':
            ...
            for key in ['model', 'lr_config', 'load_from', 'fp16', 'optimizer', 'data', 'train_pipeline', 'data_config']:
                each['init_kwargs']['config'][key] = dict(cfg._cfg_dict)[key]
            break
    ...
    if distiller_cfg is None:
        ...
        if cfg.distiller.student_init is None:
