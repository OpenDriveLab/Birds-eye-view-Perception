...
# source licenses:
# bevformer_detr_head2.py       OpenPerceptionX        Apache License, Version 2.0
#
# Contact simachonghao@pjlab.org.cn if you have any issue
#
# See:
# https://github.com/zhiqi-li/BEVFormer/blob/master/projects/mmdet3d_plugin/bevformer/dense_heads/bevformer_head.py
#
...
class BEV_FormerHeadV2(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
                 *args,
                 ...
                 num_cls_fcs=2,
                 code_weights=None,
                 learnable_code_weight=False,
                 learnable_code_weight_p=0.1,
                 only_encoder=False,
                 nms_thr=None,
                 view_positional_encoding=None,
                 bev_h=30,
                 bev_w=30,
                 **kwargs):
        ...
        super(BEV_FormerHeadV2, self).__init__(*args, transformer=transformer, **kwargs)
        self.learnable_code_weight = learnable_code_weight
        if self.learnable_code_weight:
            self.learnable_code_weight_p = learnable_code_weight_p
            self.code_weights = nn.Parameter(torch.zeros(self.code_size), requires_grad=True)
        else:
            self.code_weights = code_weights if code_weights is not None else [
        ...
        self.view_pose_encoder = build_positional_encoding(view_positional_encoding)
        self.code_weights = nn.Parameter(torch.tensor(self.code_weights, requires_grad=False), requires_grad=False)
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, return_bev=False, gt_bboxes_3d=None, **kwargs):
        ...
        for feat in mlvl_feats:
            ...
            mlvl_positional_encodings.append(self.view_pose_encoder(mlvl_masks[-1]))
    ...
    def loss_single(self, cls_scores, bbox_preds, gt_bboxes_list, gt_labels_list, gt_bboxes_ignore_list=None):
        ...
        if self.learnable_code_weight:
            bbox_weights = bbox_weights * 1 / self.code_weights.exp()
        else:
        ...
        if self.learnable_code_weight:
            loss_bbox += self.code_weights.sum() * self.learnable_code_weight_p

        # logger.info(f'code_weight: {(1 / self.code_weights.data.exp()).detach().numpy().tolist()}')
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            loss_cls = torch.nan_to_num(loss_cls)
            loss_bbox = torch.nan_to_num(loss_bbox)
        return loss_cls, loss_bbox

    # @run_time('BEV_FormerHead_loss')
    ...
    def loss(self, gt_bboxes_list, gt_labels_list, preds_dicts, gt_bboxes_ignore=None, img_metas=None):
        ...
        losses_cls, losses_bbox = multi_apply(self.loss_single, all_cls_scores, all_bbox_preds, all_gt_bboxes_list,
                                              all_gt_labels_list, all_gt_bboxes_ignore_list)
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            bboxes_for_nms = xywhr2xyxyr(bboxes.bev)
            scores = preds['scores']
            labels = preds['labels']
