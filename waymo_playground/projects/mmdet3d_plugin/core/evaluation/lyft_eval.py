...
def load_lyft_gts(lyft, data_root, eval_split, logger=None):
    ...
    for sample_token in mmcv.track_iter_progress(sample_tokens):
        #if sample_token != '1ef73991d63789118b0d90d7948baddf35bd6f7c75a07cddd7b8abe0886a48a8':
        #    continue
...
def lyft_eval(lyft, data_root, res_path, eval_set, output_dir, logger=None):
    ...
    #dist_thresholds = [0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15]
    dist_thresholds = [0.5, 1, 2, 4]
    metrics = {}
    average_precisions = \
        get_classwise_aps(gts, predictions, class_names, dist_thresholds)
...
def get_classwise_aps(gt, predictions, class_names, dist_thresholds):
    ...
    Args:
        ...
        dist_thresholds (list[float]): IOU thresholds used to calculate
            TP / FN
    Returns:
        np.ndarray: an array with an average precision per class.
    """
    #assert all([0 <= iou_th <= 1 for iou_th in dist_thresholds])

    gt_by_class_name = group_by_key(gt, 'name')
    pred_by_class_name = group_by_key(predictions, 'name')

    average_precisions = np.zeros((len(class_names), len(dist_thresholds)))
...
def get_single_class_aps(gt, predictions, dist_thresholds):
    ...
    sample_gt_checked = {
        sample_token: np.zeros((len(boxes), len(dist_thresholds)))
    ...
    tps = np.zeros((num_predictions, len(dist_thresholds)))
    fps = np.zeros((num_predictions, len(dist_thresholds)))
    ...
    for prediction_index, prediction in enumerate(predictions):
        ...
        if len(gt_boxes) > 0:
            dists = get_dist(gt_boxes, predicted_box)

            min_dist = np.min(dists)

            jmax = np.argmin(dists)

        for i, iou_threshold in enumerate(dist_thresholds):
            if min_dist < iou_threshold:
...
def get_dist(gt_boxes, predicted_box):
    dists = []
    center_pred = predicted_box.get_ground_bbox_coords().centroid
    for gt_bbox in gt_boxes:
        center_gt = gt_bbox.get_ground_bbox_coords().centroid
        dist = center_gt.distance(center_pred)
        dists.append(dist)
        #print(dir(center_gt), dir(center_pred))
    return dists
if __name__ == '__main__':

    path = 'test/bevformer_v4_lyft/Wed_Jan__5_12_48_46_2022/pts_bbox/results_lyft.json'
    #path = './test/detr3d_res101_gridmask_lyft/Wed_Jan__5_15_22_35_2022/pts_bbox/results_lyft.json'
    lyft = Lyft(
        data_path='data/lyft/v1.01-train',
        json_path='data/lyft/v1.01-train/v1.01-train',
        verbose=True)
    lyft_eval(lyft, 'data/lyft', path, 'val', '.')
