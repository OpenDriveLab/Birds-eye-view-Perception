...
#bevformer_mask = 'test/bevformer_mask3_t1/Mon_Mar_14_11_12_24_2022/pts_bbox/results_nusc.json'

bevformer_mask = 'test/bevformer_mask3_t1_raw/Sat_Apr__2_21_02_38_2022/pts_bbox/results_nusc.json'
# bevformer_mask = 'test/bevformer_mask3_t1/Sat_Apr__2_21_56_34_2022/pts_bbox/results_nusc.json'

gt = '/home/lizhiqi/bevformer/data/nuscenes-mini/nuscenes_infos_raw.pkl'
...
for ind, (pred, gt) in enumerate(zip(data1['results'], gts)):
    # print(pred)
    # break
    # print(ind)
    # print(gt.keys())
    if ind >800: break
    render_sample_data(gt, sample_toekn=gt['cam_token'][0], pred_data=data1, out_path='npys2/'+str(ind))
    #render_sample_data(gt, sample_toekn=gt['token'], pred_data=data1, out_path='npys2/' + str(ind))
    #data = np.load(f"npy2/{gt['token']}.npy")
    data = np.load(f"npy2/{gt['cam_token'][0]}.npy")
    map = np.ones([200, 200])
    car = data[0]
    verh = data[1]
    ped = data[2]
    ...
    plt.imshow(showimg, origin='lower')
    showimg = make_rgba(ped, (0.0, 0.7, 0.7))
    plt.imshow(showimg, origin='lower')
    showimg = make_rgba(verh, (0.004, 0.569, 0.969))
    plt.imshow(showimg, origin='lower')
    add_ego(showimg, np.array([-49.75, -49.75]), np.array([0.5, 0.5]))
    plt.imshow(showimg, origin='lower')
    ...
    print(showimg.shape)
    fig = plt.gcf()
    fig.set_size_inches(1000 / 300, 1000 / 300)  # dpi = 300, output = 700*700 pixels
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
    plt.gca().yaxis.set_major_locator(plt.NullLocator())
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
    plt.savefig(f'npys2/tmp_{ind}.png')
