...
data_all = np.load('.cache/mask_gt_200_200_0.50.npy')
for ind in range(0, 6009, 20):

    # print(pred)
    # break
    # print(ind)
    #if ind > 4000: break
    #render_sample_data(gt, sample_toekn=gt['cam_token'][0], pred_data=data1, out_path='npys/'+str(ind))
    #data = np.load(f"npy/{gt['token']}.npy")
    ...
    print(showimg.shape)
    fig = plt.gcf()
    fig.set_size_inches(200 / 300, 200 / 300)  # dpi = 300, output = 700*700 pixels
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
    plt.gca().yaxis.set_major_locator(plt.NullLocator())
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
    plt.savefig(f'npys/tmp_{ind}.png', bbox_inches='tight',  pad_inches=0.0)
