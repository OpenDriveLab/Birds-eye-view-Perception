...
class ResultVisualizer:
    ...
    def evaluate_and_show(self,
        ...
        # waymo_2d_bbox = mmcv.load('~/bevformer/data/waymo/kitti_format/waymo_infos_train+val+test_2d_bboxes.pkl')
        prog_bar = mmcv.ProgressBar(len(results))
        _mAPs = {}
        good_dir = osp.abspath(osp.join(show_dir, 'good'))
        bad_dir = osp.abspath(osp.join(show_dir, 'bad'))
        mmcv.mkdir_or_exist(good_dir)
        waymo_2d_bbox = defaultdict(lambda: {
            'gt_bboxes_2d': [[], [], [], [], []],
            'gt_labels_2d': [[], [], [], [], []],
        })
        for i, (result, ) in enumerate(zip(results)):
            ...
            ped = result[1]
            cyc = result[2]
            car = car[car[:, -1] > self.score_thr, :4]
            ped = ped[ped[:, -1] > self.score_thr, :4]
            cyc = cyc[cyc[:, -1] > self.score_thr, :4]
            n_car, n_ped, n_cyc = car.shape[0], ped.shape[0], cyc.shape[0]

            img_id = data_info['img_info']['id']
            key_id = img_id[:-1]
            camera_id = int(img_id[-1])
            # from IPython import embed
            # embed()
            # exit()
            waymo_2d_bbox[key_id]['gt_bboxes_2d'][camera_id] = np.concatenate((car, ped, cyc))
            waymo_2d_bbox[key_id]['gt_labels_2d'][camera_id] = ['Car']*n_car + ['Pedestrian']*n_ped + ['Cyclist'] * n_cyc
            ...
            _mAPs[i] = mAP
            save_filename = fname + '_' + str(round(mAP, 3)) + name
            out_file = osp.join(good_dir, save_filename)
            ...
            imshow_gt_det_bboxes(
                ...
                wait_time=self.wait_time,
                out_file=out_file)
        # mmcv.dump(dict(waymo_2d_bbox), 'pred_2d_bbox_0.18.pkl')
        # return
